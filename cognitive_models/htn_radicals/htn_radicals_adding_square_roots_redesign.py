...
def adding_square_root(init_value):
    ...
    answer = sp.simplify(sp.Add(sp.Mul(c1, sp.sqrt(ra), sp.sqrt(rb)), sp.Mul(c2, sp.sqrt(ra), sp.sqrt(rc))))
    hint = latex(answer)
    return tuple([
        (ans1, hint),
        (ans2, hint)
...
def htn_radicals_adding_square_roots_intermediate_hints():
    hints = {
        ...
        "evaluate_coefficient_a": ["Multiply the solved square root with the coefficient."],
        ...
        "evaluate_coefficient_b": ["Multiply the solved square root with the coefficient."],
...
htn_loaded_models.register(HTNCognitiveModel('htn_radicals_redesign',
                                             'htn_radicals_adding_square_roots_redesign',
                                             Domain,

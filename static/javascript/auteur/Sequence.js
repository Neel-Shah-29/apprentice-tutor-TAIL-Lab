...
class Sequence extends GenerationCriteriaBase {
	...
	_aggregator(variable, sequence, type) {
		...
		function searchSequence(data, seq, variableSerialized, variableSerializedKeys) {
			...
			let sequenceSerialized

			if (seq.length == 0) {
				return new Set(Array(variableSerialized[variable].length).fill().map((x,i)=>i))
			} else {
				sequenceSerialized = new RegExp(seq.map(d => {
					if (variableSerializedKeys[variable][d]) {
						return variableSerializedKeys[variable][d]
					} else {
						return "-"
					}
				}).join(""), 'g');
